...
This room is the last room of a serie of introductions to differents attacks vector, so I will use all the techniques that I learned in thoses rooms to find the flags.
...
Trying some basics login like admin:admin don't seem to work, so I tried to do basic sql injections but didn't semms to work too.
...
after login as hr, we have access to the dashboard.

![dashboard](/img/writeups/20260722-161156.png)

The first thing I got in mind after seing it was the searchbar. Firstly, I tried to see if it was vulnerable to XSS attacks but it seems not. Second thing I did was testing SQL Injection with a basic `'`. Ans this time I got and error, meining that this is vulnerable to sql injections.

![sql error](/img/writeups/20260722-161436.png)

The table has 4 columns, so we must include 4 rows too in our include. The server is using a version of MySQL with comments as `#`.

#### First injection :

`' UNION SELECT 1,database(),3,4;#`

With it, I got the database name wich is `recruit_db`
...
## Conclusion
This was a good recap room where I could practice more the SQL Injections but I was hopping to practice more the other concept we saw in the rooms before too.
